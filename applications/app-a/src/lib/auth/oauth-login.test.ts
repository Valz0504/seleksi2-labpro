...
describe('App A OAuth login initiation', () => {
  it('creates a complete authorization request without exposing the client secret', () => {
    ...
    assert.equal(authorizationUrl.searchParams.get('client_id'), 'app-a');
  ...
  it('rejects tampered, expired, or differently bound transactions', () => {
    ...
    assert.equal(
      readOAuthLoginTransaction(initiation.cookieValue, { ...config, clientId: 'app-b' }, issuedAt),
      null,
    );
  });
});
